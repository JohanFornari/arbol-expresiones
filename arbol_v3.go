...
// evalua exprecion
func comprobarNombre(nombre string) bool{
    r, _ := regexp.Compile("^[A-Z]([a-zA-Z0-9]*)_([0-9])")
    return(r.MatchString(nombre))
}
...
func main() {
	...
	for f:=0;f<3;f++{
	    scanner.Scan()
    	text = scanner.Text()
	    caracteresLeidos = strings.Split(text," ")
	    operacion=text[len(caracteresLeidos[0])+4:len(text)]
    	//fmt.Println(" \nEcuacion: \n")
        //text=pruebaLeerOperacion(text)
        //fmt.Println(text)
        ...
        if(evaluar(text)){
    	    ar=InicializarArbol(ar)
    	    fmt.Println("se leyo: ",operacion)
	        caracteres = strings.Split(operacion," ")
	        ar= guardarEnArbo(ar,caracteres)
	        fmt.Println("operacion: ",pruebaLeerOperacion(operacion))
    	    fmt.Println("\nresultado: ",caracteresLeidos[0],":= ",expresiones(ar),"\n\n")
